...
<template>
  <div class="flex flex-col sm:items-center">
    <div class="sm:w-96 m-4 md:m-6 lg:m-8" v-if="city">
      <h2 class="text-2xl font-bold">{{ city.name }}</h2>
      <RouterLink class="text-sm underline hover:no-underline opacity-80" to="/shop">
      ...
      <div class="grid gap-4 mt-4">
        ...
        <div class="bg-gray-700 rounded p-4 text-gray-200 prose prose-invert prose-sm"
             v-html="markdown.render(city.content.substring(0, 300) + `... [lees meer](https://storywalks.nl/steden/${city.id})`)"/>

        <a :href="`https://storywalks.nl/steden/${city.id}`" class="py-2 px-4 rounded bg-indigo-500 hover:bg-indigo-600">
          <div>Meer info</div>
          <div class="opacity-70 text-xs">Meer informatie, zoals de startlocatie en meer, op storywalks.nl</div>
        </a>
